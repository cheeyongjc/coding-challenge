module.exports.run = async function (movies) {
	...
	*/
	...
	//instructions do not state that error handling is needed. however, for any application I think error handling would always be necessary in case of a bad response
	const axios = require('axios');
	const movieDetails = [];
	for (i = 0; i < movies.length; i++) {
		const movieObjects = {};
		const movieList = await axios.get(movies[i])
		movieObjects['Title'] = movieList.data.Title;
		movieObjects['Year'] = movieList.data.Year;
		movieObjects['Genre'] = movieList.data.Genre;
		movieDetails.push(movieObjects);
	}
	return movieDetails;
